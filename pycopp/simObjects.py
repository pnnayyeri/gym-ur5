...
class collisionCheck(coppObject):
...
class target(coppObject):
	def __init__(self,clientID):
		super(target, self).__init__(clientID)

		self.target_handle = self.getHandle('IK_target_dummy')
